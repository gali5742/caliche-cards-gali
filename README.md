...
`/study/data` exports the complete native study database into a versioned JSON file.
...
The backup includes all six native study tables. Static textbook vocabulary is not duplicated because it remains part of the application bundle.

Restore validates the backup envelope and stored row shapes, then replaces the six native tables inside one IndexedDB transaction. If the transaction fails, the existing database is not partially replaced.
